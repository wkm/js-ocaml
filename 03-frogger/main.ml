...
let render
  ...
  let grid_to_screen (p : Position.t) = p.x * m, (config.num_rows - p.y - 1) * m in
  let interpolate_x cur next =
    ...
    then ((1. -. alpha) *. Int.to_float cur) +. (alpha *. Int.to_float next)
      ...
      let next =
        if next > cur then next - (config.num_cols * m) else next + (config.num_cols * m)
      in
...
module Scaffold_state = struct
  ...
  let render config wad screen (game_impl : 'world Game_impl.t) t =
    ...
    let alpha = if t.should_interpolate then t.interpolation_alpha -. 0.5 else 0. in
    render
      config
      wad
      screen
  ...
  let set_world_cur_and_save_history (game_impl : 'world Game_impl.t) world_cur t =
    let history =
      if game_impl.finished world_cur then t.history else t.world_cur :: t.history
    in
    ...
    Html.window##setInterval
      (Js.wrap_callback (fun () ->
         ...
         := if Float.( >= ) new_interpolation_alpha 1.0
            ...
            else { !scaffold_state with interpolation_alpha = new_interpolation_alpha }))
...
let initialize_and_main_loop (config : Config.t) wad (game : _ Game_impl.t) =
  ...
  let screen =
    { Screen.context; width = Config.width config; height = Config.height config }
  in
...
let main game =
  ...
  Html.window##.onload
  := Html.handler (fun _ ->
       let () = initialize_and_main_loop Config.default wad game in
       Js._false)
;;
